...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($repairRequestId) || empty($items)) {
        ...
        echo json_encode(['message' => 'Maintenance ID and items are required.']);
        exit();
    }
    ...
    try {
        ...
        $stmt = $conn->prepare("
            INSERT INTO material_request (maintenance_id, requested_by, status) 
            VALUES (?, ?, 'Not Started')
        ");
        ...
        $stmtRepairUpdate = $conn->prepare("
            UPDATE maintenance 
            SET maintenance_status = 'Claimed', handled_by = ?
        ...
        // Log activity
        $activity = "Processed order for maintenance ID: $repairRequestId";
        logActivity($conn, $userId, $activity, $ipAddress);

        echo json_encode(['success' => true, 'message' => 'Order processed successfully.']);
    } catch (Exception $e) {
        $conn->rollBack();
        $activity = "Failed to process order for maintenance ID: $repairRequestId";
        logActivity($conn, $userId, $activity, $ipAddress);
